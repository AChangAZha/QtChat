  ...
  for (;;)
  {
    ...
    if (socketStream.commitTransaction())
    {
      ...
      if (parseError.error == QJsonParseError::NoError)
      {
        ...
        {
          //          emit logMessage(
          //              QJsonDocument(jsonDoc).toJson(QJsonDocument::Compact));
...
void ChatClient::loginOrRegister(const QString &username, const QString &pwd, const QString &type)
{
  if (m_clientSocket->state() != QAbstractSocket::ConnectedState)
    return;
...
void ChatClient::disconnectFromHost() { m_clientSocket->disconnectFromHost(); }
