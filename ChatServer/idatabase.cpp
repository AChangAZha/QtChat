...
QJsonArray IDatabase::getLastChatRecord(QString id, QString time)
{
  ...
  {
    QJsonObject record;
    record.insert("friend", getUserName(query.value("toID").toString()));
    record.insert("sender", getUserName(query.value("id").toString()));
...
QStringList IDatabase::getGroupMember(QString id)
{
  // 在friend表中查找type为1的记录且friendID为id的记录，返回id
  QStringList memberList;
  QSqlQuery query;
  query.prepare(
      "SELECT * FROM friend WHERE type = :type AND friendID = :friendID");
  query.bindValue(":type", "1");
  query.bindValue(":friendID", id);
  query.exec();
  while (query.next())
  {
    memberList.append(query.value("id").toString());
  }
  return memberList;
}

QJsonArray IDatabase::getGroupChatRecord(QString id)
{
  QJsonArray recordList;
  QSqlQuery query;
  query.prepare("SELECT * FROM chatrecord WHERE toID = :id");
  query.bindValue(":id", id);
  query.exec();
  while (query.next())
  {
    ...
    record.insert("sender", getUserName(query.value("id").toString()));
    record.insert("toID", query.value("toID").toString());
    record.insert("to", query.value("toID").toString());
    record.insert("text", query.value("message").toString());
    record.insert("time", query.value("time").toString());
